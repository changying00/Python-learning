# 第37章：Unicode and Byte Strings（Unicode与字节字符串）

> **原书**：《Learning Python》（6th Edition），作者 Mark Lutz
> **本章地位**：第37章是字符串处理的高级篇，在第7章基础上将字符串模型扩展到完整的Unicode文本和二进制数据处理。本章是Python 3.x的必修内容，因为正常字符串本身就是Unicode。
---
...
> In 
> Chapter 7
> , the Python string story was watered down on purpose to help you
...
> reading because its normal strings simply 
> are
>  Unicode. Still, how much you
> need to care about this topic depends in large part upon which of the following
> categories you fall into:
> If you deal with non-ASCII 
> Unicode text
> —for instance, in the context
> of internet content, internationalized 
> applications, XML parsers, and
> some GUIs—you will find direct and seamless support for text
> encodings in both Python’s all-Unicode 
> str
>  object, as well as its
> Unicode-aware text files.
> If you deal with 
> binary data
> —for example, in the form of image or
> audio files, network transfers, or packed data 
> shared with lower-level
> tools—you will need to understand Python’s 
> bytes
>  object and its sharp
> distinction between text and binary data and files.
> If you fall into 
> neither
>  of the prior two categories, you may be able to
> defer this topic and use strings as you did in 
> Chapter 7
> : with the general
> str
>  object, text files, and all the familiar string operations. Your strings
...
> careers today, that impact may be more a matter of 
> when
>  than 
> if
> .
...
> Most programmers think of strings as a 
> series of characters (really, their integer
...
> For many programmers in the US, 
> ASCII
>  formed their original notion of text
> strings. ASCII is a standard 
> that defines character codes 0…127 (which always
...
> character 
> a
>  to the integer value 97 (
> 0x61
>  in hex), which can be stored in a single
> byte both in computer memory and on files.
> To witness this for yourself, Python’s built-in 
> ord
>  function shows the integer
> code of a given character; 
> chr
>  reveals the character of a given integer code; and
> hex
>  gives the code’s byte value as two hex digits, each of which fits a 4-bit
> “nibble.” The first of these, 
> ord
> , is the value of a character’s representation 
> code
> —and
>  byte—in ASCII:
> 
...
> hex(97)
>  
> # Byte value: fits 8 bits
> '0x61'
> >>> 
> 0b0111_1111
>  
> # Limit of ASCII's 7-bit range
> 127
...
> One such standard is known as 
> Latin-1
>  and is widely used in Western Europe. In
...
> (a.k.a. byte value 
> 0xc4
> ) is a specially marked and non-ASCII 
> character, 
> Ä
> . In
> Python:
...
> hex(ord('Ä'))
>  
> # Byte value in Latin-1
> '0xc4'
> >>> 
> bin(ord('Ä'))
>  
> # Latin-1 uses all 8 bits
>  
> '0b11000100'
...
> Unicode
>  provides the generality 
> we need to deal with text containing non-ASCII
> characters and symbols like these. In fact, it defines and assigns enough
> character codes
>  to represent almost every natural language in use, plus a large
> set of symbols and emojis. In Unicode speak, these codes that stand for
> characters take the form of numbers (integers), and are usually called 
> code
> points
> . The code points that Unicode assigns to characters 
> a
> , 
> Ä
> , and 
...
> Unicode is sometimes referred to 
> as “wide-character” strings because its range
...
> transmission that are both platform- and language-neutral—the 
> encodings
>  we’ll
...
> schemes like ASCII also fall under 
> the Unicode umbrella unchanged, but we
> have to move on to the next section to see how.

### 中文翻译

> 第37章把第7章里刻意简化过的字符串故事继续展开，正式进入 Unicode 文本和二进制数据的世界。Python 3 里的普通 `str` 本身就是 Unicode，所以字符串不再只是“字符序列”这么简单，而是和编码、解码、文件内容、平台默认值一起工作的一整套模型。
>
> 如果你处理的是非 ASCII 文本，比如互联网内容、国际化应用、XML 解析器或某些 GUI，那么 Python 的 `str` 和文本文件已经直接支持 Unicode 编解码，你需要真正掌握的是什么时候显式指定编码，以及如何避免把文本和字节混在一起。
>
> 如果你处理的是图像、音频、网络传输或底层工具共享的数据包，就要切换到 `bytes` 和 `bytearray` 的世界，并明确区分文本文件和二进制文件。即使你今天主要写 ASCII 文本，这套模型也值得先弄懂，因为一旦平台默认编码变化，或者数据来自别的系统，问题就会立刻暴露出来。

### 深度理解

- **核心概念**：本章内容涉及Python的核心机制
- **底层实现**：Python的内部实现涉及C语言层面的对象模型
- **设计原因**：Python的设计哲学强调可读性和简洁性
...
> character codes (a.k.a. code points) that 
> represent characters are mapped to their
...
> using an 
> encoding
> —the rules for translating a Unicode-text string into a
...
> Encoding
>  is the process of 
> translating a string of characters into its raw-
> bytes form, per any desired encoding that’s broad enough to store the
> string’s characters.
> Decoding
>  is the process of 
> translating a string of raw bytes into its
...
> memory too.
> You can view this for yourself with the 
> encode
>  method available on all Python
> text strings, which simply returns the bytes used to encode the string. The
> following means that the ASCII character 
> a
>  occupies just one byte when
> encoded per the ASCII encoding:
> >>> 
> len('a'.encode('ASCII'))
>  
> # ASCII 'a' encodes in 1 byte per ASCII
> 1
> For other encodings, the mapping can be more complex and yield multiple bytes
> per character. The widely used 
> UTF-8
>  encoding, for example, allows more
> 
> characters to be 
> represented by employing a variable-number-of-bytes scheme
...
> between 128 and 
> 0x7ff
>  (2047) are turned into two bytes, where each byte has a
> value between 128 and 255; and codes above 
> 0x7ff
>  are turned into three- or
...
> len('a'.encode('UTF-8'))
>  
> # ASCII: encodes in 1 byte
> 1
> >>> 
> len('Ä'.encode('UTF-8'))
>  
> # Non-ASCII: encodes in 2 bytes
> 2
> >>> 
> len('
> '.encode('UTF-8'))
>  
> # Emoji: encodes in 4 bytes
> 4
> Despite such details, it’s important to note that ASCII is a 
> subset
>  of both Latin-1
> and UTF-8. This is 
> true because these encodings encode ASCII characters to
...
> than ASCII to encode at all (the 
> b'…'
>  here is the Python 
> bytes
>  object, which
...
>  
>  
> # ASCII encodes to the same byte
>  
> b'a'
> >>> 
> 'a'.encode('UTF-8')
...
> 'Ä'.encode('UTF-8')
>  
> # But non-ASCII requires more bytes
> b'\xc3\x84'
> >>> 
> '
> '.encode('UTF-8')
>  
> # And more inclusive encodings
> b'\xf0\x9f\x99\x82'
> >>> 
> '
> '.encode('ASCII')
...
> Other encodings support richer character sets in other ways. For instance, 
> UTF-
> 16
>  and 
> UTF-32
>  use a fixed and larger 2 and 4 bytes per character, respectively,
> the former 
> with a special 
> surrogate-pair
>  protocol for codes too large for 2 bytes.
> Both of these, along with UTF-8, may also allow or require a 
> BOM
>  (Byte Order
> Marker) preamble at the 
> start of encoded text, which can designate byte order
...
> decoded. The net effect ensures that text is 
> portable
>  across all the tools that use it
> in exchange for minor translation costs:
> When 
> decoded
> , character code points may or may not occupy multiple
...
> When 
> encoded
> , the format of character code points is wholly
...
> box; see the 
> codecs
>  module in the Python Library Reference for a list. Importing
> module 
> encodings
>  and asking for 
> help(encodings)
>  shows you many as well.
...
> Latin-1
> . We’ll revisit encodings later in this chapter
> when we study Unicode coding techniques.
> For another take on the Unicode backstory, see 
> the Python standard manual at
> python.org
> . It includes a 
> Unicode HOWTO section
...
> types to represent content in your script: both 
> textual data
> —integer code-point
> values of decoded Unicode 
> characters in 
> memory—as
>  well as 
> binary data
...
>  type (e.g., 
> 'text'
> ) is for decoded Unicode text. It’s
...
> In memory, a 
> str
>  is just an ordered collection of Unicode code-point integers,
> which 
> print as 
> glyph
> s—visual representations that may vary from host to host—
> of the characters that the code points represent. When transferred to and from
> files, a 
> str
>  is automatically encoded to and decoded from a sequence of bytes
> using either the host platform’s default or a provided encoding name to translate
> with an explicit scheme. 
> str
>  objects themselves, however, have 
> no notion of an
> encoding
> ; they are just character code points.
> The bytes Object
> While 
> str
>  is great for Unicode text, many programs need to process raw binary
> content that is not 
> encoded per any Unicode format—as well as the bytes used to
> store text when it is encoded. Image files and packed data you might process
> with Python’s 
> struct
>  module fall into this category. To accommodate this, the
> bytes
>  type supports processing of truly binary data. 
> bytes
>  is just raw bytes, not
...
> The 
> bytes
>  type is formally defined 
> as an 
> immutable sequence of 8-bit integers
> .
> Its content represents byte values, and it supports almost all the same operations
> that the 
> str
>  type does; this includes string methods, sequence operations, and
> even 
> re
>  module pattern matching (formatting works on 
> bytes
>  today, too, but
...
> range 0…255: indexing a 
> bytes
>  returns an 
> int
> , slicing one returns another
> bytes
> , and running 
> list
>  on one returns a list of integers, not characters.
> However, when processed with operations that assume characters (e.g., the
> 
> isalpha
>  method), the contents of 
> bytes
>  objects are assumed to be ASCII-
> encoded bytes. Further, 
> bytes
>  items whose values fall in the range of ASCII
...
> Though less commonly used, Python 
> also comes with 
> bytearray
> , a variant of
> bytes
>  that is 
> mutable
>  and so supports in-place changes. The 
> bytearray
>  type
> provides the usual string operations that 
> str
>  and 
> bytes
>  do but also has many of
> the same in-place change operations as lists (e.g., 
> append
>  and 
> extend
>  methods
...
> bytearray
>  adds direct in-place mutability for string data—something long
> prohibited by 
> str
>  and 
> bytes
...
> Because file I/O is one of the 
> main benefactors of encodings, it’s also a core
> Unicode tool. As we’ve seen, text is really just decoded integer character codes
> when it is in memory; it’s when text is transferred to and from 
> external
> interfaces
>  like files that Unicode encodings come into play. By contrast, truly
...
> binary files accessed with the built-in 
> open
>  
> function
> :
...
> text mode
> , reading its data automatically
> decodes its content and returns it as a 
> st
> r, and writing takes a 
> str
>  and
> automatically encodes it before transferring it to the file. In both cases,
> the encoding to use is either a platform default or a provided 
> encoding
> argument to 
> open
> . Text mode files also support universal newline (a.k.a.
...
> argument in the 
> open
>  call, reading its data does not decode it in any way
> and simply returns its content raw and unchanged as a 
> bytes
>  object.
> 
> Writing similarly takes a 
> bytes
>  object and transfers it to the file
> unchanged, and binary-mode files also accept a 
> bytearray
>  object for
...
>  and 
> bytes
>  are sharply differentiated by the language this way, you
> must decide whether your data is text or binary in nature and use 
> str
>  or 
> bytes
> objects to represent its content in your script, respectively. Ultimately, the mode
> in which you open a file will dictate which type of 
> object
>  your script will use to
...
> streams, chances are good that you will want to deal with it using 
> bytes
> and binary-mode files. You might also opt for 
> bytearray
>  to update the
...
> str
>  and text-mode files.
> Subtly, the mode-string argument to 
> open
>  (its 
> mode
>  keyword and second
...
> processing 
> mode
>  but also implies a Python object 
> type
> . By adding a 
> b
>  (lowercase
...
> usually provide, a 
> bytes
>  object to represent the file’s content when reading or
> writing. Without the 
> b
> , your file is processed in text mode, and you’ll use 
> str
> objects to represent its content. For example, modes 
> rb
> , 
> wb
> , and 
...
> to explore 
> string operations as they extend to Unicode and bytes.
> Using Text Strings
...
> Most Python string objects are born when 
> you call a built-in function such as
> str
>  or 
> bytes
> , process a file created by calling 
> open
> , or code literal syntax in
> your script. For the latter, the usual 
> '…'
>  makes a 
> str
> ; a unique literal form 
> b'…'
> is used to create a 
> bytes
> ; and 
> bytearray
>  objects may be made by calling the
...
> "…"
> ,
> and triple-quoted blocks—generate a 
> str
> ; adding a 
> b
>  or 
...
> r'…'
>  raw string we’ve also met, which suppresses
...
> S = 'hack'
>  
> # Make a str object: Unicode characters
> >>> 
> type(B), type(S)
...
> B
>  
> # Sequence of ints, prints as ASCII characters
> b'code'
> >>> 
> S
>  
>  
> # Sequence of code points, prints as text glyphs
> 'hack'
> >>> 
> B2 = B"""
>  
> # bytes prefix works on single, double, triple
> xxxx
> yyyy
> """
> >>> 
...
> B[1:], S[1:]
>  
> # Slicing makes another bytes or str
> (b'ode', 'ack')
> >>> 
> list(B), list(S)
...
>  and more about 
> bytes
>  to a dedicated section later in this
> chapter.
> NOTE
> Blast from the past
> : Python 3.X also recognizes Python 2.X’s Unicode string literals to ease
> migration of 2.X code: a 2.X 
> u'…'
>  literal in Python 3.X is just a synonym for a 3.X 
> '…' str
> literal. This makes sense, given that 3.X’s 
> str
>  is all Unicode, and allows some 3.X code to run
> on 2.X and vice versa. In today’s 3.X world, though, there’s no compelling reason to use the
> u'…'
>  literal anymore (unless you’re a fan of superfluous prefixes), but it may crop up in
> Python code you’ll encounter in 
> the wild. 2.X had a very long shelf life, after all.
> String Type Conversions
> Syntax aside, the first thing you 
> might notice about Python strings is what they
> cannot
>  do—
> str
>  and 
> bytes
>  never mix automatically in expressions and generally
> are not converted to one another automatically when passed to functions. A
> function that expects an argument to be a 
> str
>  may not accept a 
> bytes
>  (and vice
...
>  is: if the 
> bytes
>  is encoded text, its encoding is unknown, but
...
> default. The methods’ default is always UTF-8 (by contrast, 
> open
>  uses a value in
> the 
> locale
>  module you’ll meet shortly that may vary per platform, settings, and
...
> # str + str (code points)
> 'hackcode'
> A few cautions on defaults here. First of all, the encoding argument to 
> bytes
>  is
> not optional
...
> off in 
> str
>  calls does not mean it defaults—instead, due to Python history, a 
> str
> without an encoding returns the 
> bytes
>  object’s 
> print string
> , not its decoded and
> converted 
> str
>  form (this is usually not what you’ll want!).
> Assuming again that 
> B
>  and 
> S
...
> str(B)
>  
> # str() works without encoding
> "b'code'" 
> # But print string, not conversion!
> >>> 
> len(str(B))
> 7
> >>> 
...
>  
> # Pass encoding to convert to str
> 4
...
>  and 
> locale
>  modules but aren’t as trustworthy as you might
> think:
> $ 
> py -3
> >>> 
...
> sys.platform
>  
> # Underlying platform: Windows
> 'win32'
> >>> 
> sys.getdefaultencoding()
>  
> # Methods default (but not for str()!)
> 'utf-8'
> 
> >>> 
> locale.getpreferredencoding(False)
>  
> # open() default: a Latin-1 superset
> 'cp1252'
> As shown, the 
> open
>  function’s default file encoding lives in the 
> locale
>  module.
> On the PC used for Windows examples in this chapter, it’s 
> cp1252
> —a superset
> of Latin-1 that adds characters like slanted quotes. Defaults may differ, however,
> on other 
> platforms
>  and technically can even depend on environment-variable
...
> sys.platform
>  
> # Underlying platform: macOS
> 'darwin'
> >>> 
> sys.getdefaultencoding()
>  
> # The default for methods is same
> 'utf-8'
> >>> 
> locale.getpreferredencoding(False)
>  
> # But this differs on Unix: be explicit!
> 'utf-8'
> Besides such program-host differences, keep in mind that text 
> content
>  you
...
> generally rely on 
> open
>  defaults if they may need to care about portability now or
> in the future—always pass an explicit encoding to 
> open
>  when interoperability
> counts. We’ll revisit encoding defaults and learn how to provide an explicit
> encoding to 
> open
>  when we explore Unicode files later in this chapter.
...
> conversion 
> of sorts, and the mapping may seem trivial for simple text like
...
> 
> Encoding and decoding grow 
> more meaningful when you start dealing with non-
...
> \x
> NN
>  hex escapes, where two hex digits (
> NN
> ) specify a character code as
...
> \U
> NNNNNNNN
>  Unicode escapes, where the first 
> lowercase
> form gives 4 hex digits to denote a 2-byte (16-bit) character code, and
> the second 
> uppercase
>  form gives 8 hex digits for a 
> 4-byte
>  (32-bit) code
> Importantly, in 
> str
>  objects, all three of these escapes are used to give a Unicode
> character’s 
> code-point
>  value—not its encoded bytes. By contrast, 
> bytes
>  objects
> allow only hex escapes for byte values; for text, this gives its 
> encoded form
> —not
> its decoded code points.
> Let’s see how this all translates to code. Simple 7-bit 
> ASCII
>  text is formatted
...
> ord('X')
>  
> # Character 'X' has code-point value 88
>  
> 88
> >>> 
> chr(88)
>  
> # Code-point 88 stands for character 'X'
> 'X'
> >>> 
> S = 'XYZ'
>  
> # str: code points display as their character glyphs
> >>> 
> S
> 'XYZ'
> >>> 
> len(S)
>  
> # 3 characters (not necessarily bytes) long
> 3
> >>> 
> S.encode('ascii')
>  
> # Values 0…127 in 1 byte each (ASCII shown as chars)
> b'XYZ'
> >>> 
> S.encode('latin-1')
>  
> # Values 0…255 in 1 byte each
> b'XYZ'
> >>> 
> S.encode('utf-8')
>  
> # Values 0…127 in 1 byte, 128…2047 in 2, others 3~4
> b'XYZ'
> By contrast, the less 
> common 
> UTF-16
>  and 
> UTF-32
>  use 2 and 4 bytes for every
...
> print as characters, non-ASCIIs print as 
> \x
> NN
>  escapes, padding bytes follow text,
...
> S.encode('utf-16')
>  
> # Always 2 or 4 bytes per character, with BOM header
...
> To code 
> non-ASCII
>  characters, you can use hex and Unicode escapes in your
> strings. The numeric values coded 
...
> outside the 7-bit range of ASCII; we can embed them in 
> str
>  objects anyhow
> because 
> str
>  supports Unicode in full:
> >>> 
> chr(0xc4)
>  
> # 0xC4 and 0xE8 are accented characters outside ASCII
> 'Ä'
> >>> 
> chr(0xe8)
> 'è'
...
> S = '\xc4\xe8'
>  
> # Hex escapes: code-point values, not encoded bytes
> >>> 
> S
> 'Äè'
> >>> 
> S = '\u00c4\u00e8'
>  
> # Unicode escapes: 16-bits (2-bytes)
> >>> 
> S
> 'Äè'
> >>> 
...
> 4
> You can also go 
> the other way—from raw bytes back to a Unicode string. You
> could read raw bytes from a file and decode manually this way, but the encoding
> mode you give to the 
> open
>  call causes this decoding to be done for you
...
> values for characters in your 
> str
>  strings: use 
> \u…
>  with 4 hex digits for the former
> and 
> \U…
>  with 8 hex digits for the latter. As the last example in the following
> shows, you can also build such strings up piecemeal using 
> chr
> , but it might
...
> points to be coded 
> with both hex and Unicode escapes in 
> str
>  string literals but
> allows only 
> hex
>  escapes in 
> bytes
>  literals—and prints a warning in its recent
...
> bytes
>  objects hold binary data, both
> textual and not. When they contain text, they hold characters’ 
> encoded
>  bytes—
> not their decoded code points. Thus, Unicode code-point escapes simply don’t
> apply to 
> bytes
> , and hex escapes in their literals yield raw byte values, not
> 
> characters.
> This is true even though code-point and encoded-byte values happen to be the
> same for some characters in some encodings (confusingly!). Because 
> bytes
>  are
> not code points, they also must be decoded to 
> str
>  to print their non-ASCII
> characters properly:
> >>> 
> S = 'A\xC4B\xE8C'
>  
...
> S = 'A\u00C4B\U000000E8C'
>  
> # 4- and 8-digit Unicode escapes (str only)
> >>> S
> 'AÄBèC'
> >>> 
> B = b'A\xC4B\xE8C'
>  
> # bytes recognizes hex escapes, but not Unicode
> >>> 
> B
> b'A\xc4B\xe8C'
> >>> 
> B = b'A\u00C4B\U000000E8C'
>  
> # Unicode escape sequences taken literally!
> <stdin>:1: SyntaxWarning: invalid escape sequence '\u'
> >>> 
> B
>  
> # bytes is encoded bytes, not code points
> b'A\\u00C4B\\U000000E8C'
> >>> 
> B = b'A\xC4B\xE8C'
>  
> # Use hex escapes for latin-1 bytes
> >>> 
> B
>  
> # Prints non-ASCII bytes as hex
>  
> b'A\xc4B\xe8C'
> >>> 
> print(B)
>  
>  
> # For both interactive and print()
> b'A\xc4B\xe8C'
> >>> 
> B.decode('latin-1')
>  
> # Decode to str to interpret as text
>  
> 'AÄBèC'
> Finally, notice that 
> bytes
>  literals assume that textual characters embedded
> within them are ASCII and require escapes for byte values > 127. By contrast,
> str
>  literals in code like that in the following allow embedding any character
...
>  
>  
> # Decoded to str when code is read by Py
> 'AÄBèC'
> >>> 
> B = b'AÄBèC'
> 
...
> B = b'A\xC4B\xE8C'
>  
> # Chars must be ASCII, or hex escapes
> >>> 
> B
>  
> # Non-ASCIIs are latin-1 encoded bytes
> b'A\xc4B\xe8C'
> >>> 
> B.decode('latin-1')
> 'AÄBèC'
...
>  
> b'A\xc3\x84B\xc3\xa8C' 
> # Methods use UTF-8 to encode, unless passed
> >>> 
> S.encode('utf-8')
> b'A\xc3\x84B\xc3\xa8C'
> >>> 
...
> S.encode()
>  
> # Method's default utf-8 encoding
> b'A\xc3\x84B\xc3\xa8C'
> >>>
> >>> 
> T = S.encode('cp500')
...
> U.encode()
>  
> # Back to UTF-8 bytes, by default
> b'A\xc3\x84B\xc3\xa8C'
...
> encoded with a decode (to 
> str
> ) plus an encode (to 
> bytes
> ) combination that
> changes its stored encoding; as you’ll see ahead, the 
> open
>  function does most of
> this work for you.
> Also, note how the preceding code is able to use a 
> str
>  literal 
> 'AÄBèC'
>  with raw
...
> coding 
> escapes and works as long as your code file (and GUI) support it, as the
...
> but they can become tedious if 
> you need to code non-ASCII text in your strings
> frequently. For string literals and other text that you embed in your script files
> (or paste into your coding GUI), Python uses the 
> UTF-8
>  encoding by default to
...
> characters that the chosen encoding 
> supports
> .
...
> (e.g., a 
> #!
>  line works before it: see 
> Appendix A
> ) and is usually of the following
...
> text editor that accepts, displays, and saves 
> accented
>  and other non-ASCII
...
> #----------------------------------------------------------------------------
> myStr1 = 'AÄBèC' 
> # Raw, per source encoding
> myStr2 = 'A\xc4B\xe8C' 
> # Hex code-point escapes
> myStr3 = 'A\u00c4B\U000000e8C' 
> # Unicode short/long escapes
> myStr4 = 'A' + chr(0xC4) + 'B' + chr(0xE8) + 'C' 
> # Concatenated code points
> import sys, locale
...
> # Decoded text+length
>  bytes1 = aStr.encode() 
> # Default UTF-8: 2 bytes for accents
>  bytes2 = aStr.encode('latin-1') 
> # Explicit Latin-1: 1 byte per char 
...
> —the 
> encode
>  method’s default UTF-8, and an explicit Latin-1 (ASCII is too
...
> > myStr1 = 'AÄBèC'
> $ 
> python3 source-encoding-utf8.py
>  
> # Same output as Latin-1 version above
> Since most programmers are likely to fall back on the default and general (really,
> universal
> ) UTF-8 encoding in Python, we’ll defer to Python’s standard 
> manual
> set
>  for more details on this option, as well as its more advanced and obscure
> Unicode support such as properties and character-name escapes in strings that
> we’ll skip here.
> NOTE
> Unicode in variable names
> : Source-file encoding declarations apply to a file’s content in
> general and support arbitrary kinds of text. The rules for variable names within a file’s code,
> however, are more stringent.
> As noted briefly in 
> Chapter 11
> , Python allows some, but not all, non-ASCII Unicode characters
> to be used for variables in your code. Roughly, number- and letter-like characters work, but
> symbols and emojis are not allowed. For instance, 
> hÄck
>  is a valid variable, but 
> hÄck
>  is not.
> You can check whether a specific string passes as a variable with the 
> isidentifier
>  method of
> str
> , but the rules behind this are complex and best had in Python’s language manual.
> 
...
> code in 
> the Python standard library must use ASCII-only identifiers for this reason. As usual,
> use with care.
> Using Byte Strings
> We’ll be able to see strings in action 
> again when we study files ahead. First,
> though, let’s take a brief side trip to dig a bit deeper into the operation sets
> provided by objects geared for binary data—the 
> bytes
>  type and its 
> bytearray
...
> bytes
>  type supports sequence operations and
> most of the same methods available on 
> str
> . Even so, because you cannot mix
> and match 
> bytes
>  and 
> str
>  without explicit conversions, you’ll generally use 
> str
> objects and text files for text data and 
> bytes
>  objects and binary files for binary
> data. This makes 
> bytes
>  a crucial tool in many roles and worthy of a quick demo
> here.
> Methods
...
>  but not in
> Y
> ). This can also tell you something about the expression operators they support
> (e.g., 
> __mod__
>  and 
> __rmod__
>  implement the 
> %
>  operator, and they’re present in
> both today):
> $ 
> python3
...
> attributes are generally methods that don’t apply to the other (
> format
>  is an
> outlier you’ll meet shortly). For instance, 
> decode
>  translates a raw 
> bytes
>  into its
...
> bytes
> representation
> . Most other methods are shared between the two types. Moreover,
> bytes
>  are immutable just like 
> str
> :
> >>> 
> B = b'code'
...
>  
>  
> # Search for substr offset
> 1
> >>> 
> B.replace(b'od', b'XY')
>  
>  
> # New bytes with replacement
> b'cXYe'
> >>> 
> B
> b'code'
...
> For more 
> bytes
>  methods, see the earlier coverage of string fundamentals in
> Chapter 7
> ; 
> bytes
>  do most of the same work, though their methods generally
> return a new 
> bytes
>  instead of a 
> str
> .
> Sequence Operations
> Besides method calls, all the usual 
> generic sequence operations you know from
> other sequences, like lists, work as expected on both 
> str
>  and 
> bytes
> . This
> includes indexing, slicing, concatenation, and so on. As we’ve learned, 
> str
>  is a
> sequence of character code points, and 
> bytes
>  is a sequence of byte-size integers,
> but their sequence operations’ semantics are the same.
> Notice in the following, though, that indexing 
> bytes
>  returns an integer giving
> the byte’s binary value; 
> bytes
>  really is a sequence of 8-bit integers in the 0…
...
>  
>  
> # bytes and str are both sequences
> >>> 
> B
>  
>  
> # Bytes in the 0…127 range display as ASCII
> b'code'
> >>> 
> B[0], B[-1]
>  
> # Indexing: returns a byte's integer value
> (99, 101)
> >>> 
> 'code'[0]
>  
> # But indexing a str returns a 1-item str
> 'c'
> >>> 
> chr(B[0])
>  
> # Unicode character (code point) for byte's value
> 'c'
> >>> 
> list(B)
>  
> # But it's really integer bytes
> [99, 111, 100, 101]
> >>>
>  
> b'A\x42C\xFF\x63'
>  
> # That happen to display as ASCII if in 0…127
> b'ABC\xffc'
> >>> 
> chr(0x63), hex(B[0])
>  
> # And accept hex escapes for byte values
> ('c', '0x63')
> >>> 
> B[1:], B[:-1]
>  
> # Slicing: bytes (that display 0…127 as ASCII)
> (b'ode', b'cod')
> >>> 
> len(B)
>  
> # Length: number bytes (not necessarily characters)
> 4
> >>> 
> B + b'lmn'
>  
...
> b'codecodecodecode'
> Formatting
> One notable exception to the same-operations 
> rule for strings: string formatting 
> %
> expressions work on 
> bytes
>  too (as of Python 3.5), but neither the 
> format
> method nor 
> f'…'
>  f-string formatting is available for 
> bytes
> —an odd bifurcation
...
> b'a %s string' % b'fine'
>  
> # Py 3.5+ formatting for bytes
> b'a fine string'
> >>> 
> b'a %s string' % bytes([0xFF, 0xFE])
>  
> # Non-ASCII bytes work too
> b'a \xff\xfe string'
> >>> 
> 'a {} string'.format('fine')
>  
> # But format method only for str
> 'a fine string'
> >>> 
...
> f'a {kind} string'
>  
> # But f-strings only for str
> 'a fine string'
> >>> 
> bf'a {kind} string'
> SyntaxError: invalid syntax
> There are arguably sound reasons that formatting shouldn’t work on 
> bytes
> —it’s
...
> violated for the sake of the 
> %
>  expression alone.
> Moreover, post-operation conversion by encoding isn’t the same as 
> bytes
...
>  
> # Encoding != bytes operations
> b'a fine string'
> >>> 
> kind = 'AÄBèC'
>  
> # And narrow encodings may fail
> >>> 
...
> So far in this section, we’ve been making 
> bytes
>  objects with the 
> b'…'
>  literal
> syntax, but they 
> can also be created by calling the 
> bytes
>  constructor with a 
> str
> and an encoding name, by calling 
> bytes
>  with an iterable of integers representing
> byte values, or by encoding a 
> str
...
> 
> For example, encoding takes a 
> str
>  and returns the raw binary 
> bytes
>  value of the
> string according to its encoding specification; decoding takes a raw 
> bytes
...
>  and 
> bytes
> , as expanded upon in the next section.
> Mixing String Types
> When we used the 
> replace
>  earlier when sampling 
> bytes
>  methods, you may
> have noticed 
> that we had to pass in two 
> bytes
>  objects for from and to—
> str
...
> different types of data altogether. Second, although you can create 
> bytes
>  objects
...
> though, let’s briefly explore 
> bytes
> ’ elusive and changeable colleague.
> The bytearray Object
...
>  and 
> bytes
>  because they will be your
> go-to string tools. Python, however, has a third string type—
> bytearray
> , which
> is essentially a mutable variant of 
> bytes
> , and thus a mutable sequence of
> integers in the range 0…255. As such, it supports the same string methods and
> sequence operations as 
> bytes
> , as well as the mutable in-place-change operations
> found on lists.
> We’ve already seen most operations that apply to 
> bytearray
> , so we’ll just take a
> 
> quick tour here to sample their flavor. First off, you can call 
> bytearray
>  as a
> function passing a 
> bytes
>  (not a 
> str
> ) to make a new mutable sequence of small
> (0…255) integers:
> >>> 
> B = b'code'
>  
> # A str 'code' does not work: not bytes
> >>> 
> C = bytearray(B)
> >>> 
> C
...
> C[0], chr(C[0])
>  
> # ASCII code-point integer for 'c'
> (99, 'c')
> Once you’ve got a 
> bytearray
> , you can change it in place using the same sorts of
> operations available to modify a list, but keep in mind that you must assign just
> integers to its cells, not 
> str
>  text strings, and not arbitrary objects:
> >>> 
> C[0] = 'x'
...
> The 
> bytearray
> ’s methods set overlaps broadly with both 
> str
>  and 
> bytes
...
>  or 
> bytearray
>  by passing in any sort of integer sequence or
> generator, not just text strings. This also provides a sort of conversion from an
> integer character code to a 
> bytes
> , but only if the integer is in the range 0…255,
> and only if it’s embedded in a sequence (else you get a 
> bytes
>  with that many
...
> >>> 
> bytes(range(97, 104)), bytes(c + 97 for c in range(7))
>  
> # Generators too
> (b'abcdefg', b'abcdefg')
...
> content and support many of the same operations, you should use 
> str
>  for
> decoded text, 
> bytes
>  for binary data including encoded text, and 
> bytearray
>  for
...
> mode
>  in which you open a file is crucial in Python: it
...
> implies 
> str
>  objects and binary mode implies 
> bytes
> , as follows:
> Text-mode files
> Interpret file contents according 
> to an encoding—either the default for your
> platform or one whose name you pass in to 
> open
> . By passing in an encoding
> name, you can force conversions for various types of Unicode files. Text-
> mode files may also handle BOM headers 
> for some encodings (deferred till
...
> Binary-mode files
> Instead return file content to you raw as a 
> sequence of integers representing
> byte values, with no encoding or decoding, no BOM handling, and no
> newline translations.
> In terms of code, the second positional argument to 
> open
>  (a.k.a. 
> mode
>  when
...
> types—adding a 
> b
>  to the mode string implies binary mode. The default mode is
> rt
> , which is the same as 
> r
> , and means text input. In addition, the mode argument
> 
> to 
> open
>  also implies an object type for file content representation regardless of
> the underlying platform—text files return a 
> str
>  for reads and expect one for
> writes, but binary files return a 
> bytes
>  for reads and expect 
> bytes
>  (or
> bytearray
> ) for writes.
...
>  
> # Use default encoding on host, mode w=write
> >>> 
> size = file.write('abc\n')
>  
>  
...
> text
> 'abc\n'
> As a refresher, the first argument to 
> open
>  is the file’s 
> pathname
> —the address of
> a file in the host’s folder hierarchy that’s either absolute or relative to the 
> current
> directory. The second argument to open is 
> mode
> —where 
> w
>  means write text, and
> the default means read it, and 
> write
>  methods return the number of written 
> items
> —either characters for text mode or bytes for binary mode. Also, the 
> close
>  call
> here is optional in some contexts (e.g., the widely used 
> CPython
>  auto-closes files
> when their objects are garbage collected) but is generally advised to flush
> changes and avoid memory growth.
> 1
...
> handling for 
> \n
> , but we must move ahead for more on encodings and 
> newlines
> .
...
> text file
>  and read 
> it back in both text and binary modes. Notice
> in the following how text mode requires us to provide a 
> str
>  for writing, 
> rb
> distinguishes binary-mode input, and reading gives us a 
> str
>  or 
> bytes
>  depending
> on the mode (opens and transfer operations are strung together here into one-
> liners just for brevity; again, remember to 
> close
...
>  but 
> \r\n
>  when reading in binary mode with 
> bytes
> . This
...
> Now, let’s do the same, but with a 
> binary file
> . We provide a 
> bytes
>  to write and
...
>  or 
> bytes
>  depending on the input mode, though the 
> \n
>  isn’t
> expanded to 
> \r\n
...
>  
> # Text-mode input, receive a str
> 'abc\n'
> >>> 
> open('temp.bin', 'rb').read()
>  
> # Binary-mode input, bytes sans mapping
> b'abc\n'
> This holds true even if the data we’re writing to the binary file is truly binary in
> nature. In the following, the 
> \x00
>  is a binary zero byte and not a printable
> 
> character, though it works in the middle of a 
> bytes
>  and qualifies as a text code
...
>  
> # Binary data included
> 3
> >>> 
> open('temp.bin', 'r').read()
>  
> # Text-mode: str with NUL
> 'a\x00c'
> >>> 
> open('temp.bin', 'rb').read()
>  
> # Binary mode: bytes
> b'a\x00c'
> Binary mode files always 
> return contents as a 
> bytes
>  object but accept either a
> bytes
>  or 
> bytearray
>  object for writing. This naturally follows, given that
> bytearray
>  is mostly just a mutable variant of 
> bytes
> . In fact, most APIs in
> Python that accept a 
> bytes
>  also allow a 
> bytearray
>  (the bytes here are also
...
> open('temp.bin', 'rb').read()
> b'\x01\x02\x03'
> Finally, notice that you can’t get away with violating Python’s 
> str
> /
> bytes
>  (i.e.,
...
>  or 
> str
>  when used in the opposite context and wisely refuses
> to convert implicitly (a 
> bytes
>  might be an image, after all). Moreover, because
> str
>  and 
> bytes
>  operation sets largely intersect, the choice of types won’t be
> much of a dilemma for most programs. Watch for the 
> struct
>  module coverage
> ahead for another binary-file example.
> Unicode-Text Files
> And now for the featured attraction 
> of our files tour: text files with non-ASCII
...
> specific Unicode encoding-scheme name to be passed in with an 
> encoding
> argument to 
> open
...
> encoded per UTF-8 and automatically decodes it to 
> str
>  Unicode code points
> when read by the program. Similarly, the result of the second line of code
> encodes 
> str
>  code points to their Latin-1 format as they are output to the file.
> Here’s the text-file story with the universal 
> UTF-8
>  encoding and three non-
...
> the encoding you pass must match the data. For example, 
> ASCII
>  is not inclusive
...
> '0x1f49b'
> By contrast, using the broader and more general 
> UTF-16
...
> Even for broader encodings like UTF-8 and UTF-16, though, you cannot 
> mix
> and match
> : using an incompatible encoding still won’t work because encoded
> bytes in the file differ. Although you can sometimes handle unknown encodings
> with binary-mode files, 
> open
>  error handlers, and other techniques, your encoding
...
>  of an 
> encoding
>  argument, text files still encode and decode per a
> host- and platform-specific default in 
> locale
>  discussed earlier. But as a
...
> portability issues (and nightmares; see the upcoming sidebar 
> “Unicode Defaults
> and UTF-8 Mode”
> ).
...
> fetched from other devices may be encoded arbitrarily. Use explicit 
> open
...
> (again, its default is the same as 
> r
> ) and then passes mode by explicit keyword
...
> Once you’ve loaded content this way, all the 
> str
>  and 
> bytes
>  operations we’ve
...
> party 
> Pillow
>  (f.k.a. PIL) imaging library for Python or similar. With Python’s
> files and strings, content possibilities are largely endless.
> In the interest of full disclosure, Python’s 
> open
>  accepts additional arguments that
> modify its behavior. Among them, 
> newline
>  changes newline mapping; 
> errors
> specifies handling of encoding and decoding errors (e.g., 
> 'surrogateescape'
> replaces failing bytes with sequences that can be used to restore them on output);
> and 
> buffering
>  alters, well, buffering. Because their defaults are generally what
...
>  module also has an 
> open
>  that returns an auto-
> decode/encode file object just like the built-in 
> open
>  function. This was used for Python 2.X
> backward compatibility in times gone by, but there’s no 
> obvious reason to rely on it in new
...
> 理解 Unicode 的关键，是先把“字符是什么”和“字符怎么存”分开。Unicode 负责给字符分配码点，也就是给每个字符一个整数身份；而编码负责把这些码点翻译成适合存储和传输的字节序列。
>
> 在 Python 里，`encode()` 和 `decode()` 正是这两个方向的转换：前者把 `str` 变成原始字节，后者把字节还原成文本。ASCII 和 Latin-1 这类编码几乎是一一映射，所以转换很直接；UTF-8 则使用可变长度字节方案，能表示更多字符，也更适合今天的网络和文件系统。
>
> 这也是为什么 `ord`、`chr`、`hex`、`bin` 这些工具很有用：它们让你看到文本背后的数值表示，从而明白字符、码点和字节并不是同一个层面的东西。只要把这层区分清楚，后面的编码问题就不会显得神秘了。
...
> /
> bytes
>  dichotomy. We won’t cover any of these application-focused topics in
> detail in this core-language book, but as a sample, here’s a 
> very
>  brief look at how
...
> re
>  pattern-matching 
> module has been generalized to work on objects of
> any string type—
> str
> , 
> bytes
...
> in, and you can’t mix 
> str
>  and 
> bytes
>  types in its calls’ arguments (sans explicit
> conversions, of course); 
> bytearray
>  is also unusable here as a pattern because
...
> struct
>  module, used to 
> create and extract packed binary data from
> strings, operates on 
> bytes
>  and 
> bytearray
...
> >>> 
> data = struct.pack('>i4sh', 7, b'code', 8)
>  
...
> pickle
>  module 
> provides another way to save data in files but allows
> saved data to be nearly arbitrary Python objects instead of values coerced to
> bytes as in 
> struct
> . We met this module briefly in Chapters 
> 9
> , 
> 28
> , and 
> 31
> . For
> completeness here, keep in mind that the 
> pickle
>  module always creates a 
> bytes
> object, regardless of the default or passed-in 
> protocol
>  (data format level). You
> can see this by using the module’s 
> dumps
>  call to return an object’s pickle string:
> >>> 
> import pickle
>  
...
> b'\x80\x04\x95\x15\x00\x00\x00\x00 …
> etc
> … \x04\x8c\x04\xf0\x9f\x90\x8d\x94e.'
> >>> 
> pickle.dumps(['code', 4, '
> '], protocol=0)
>  
> # ASCII protocol 0, still bytes!
> b'(lp0\nVcode\np1\naI4\naV\\U0001f40d\np2\na.'
> This implies that files used to store pickled objects must always be opened in
> binary mode
>  because text files use 
> str
>  strings to represent data, not 
> bytes
> , and
> the 
> dump
>  call simply attempts to write the pickled byte string to an open output
> file:
> >>> 
> pickle.dump(['code', 4, '
> '], open('temp.pkl', 'w'))
>  
> # bytes+text mode fail
> TypeError: write() argument must be str, not bytes 
> >>> 
> pickle.dump(['code', 4, '
...
> Chapter 9
> , is a related tool: it converts a
> nested-object tree to a text string that can be saved on a file to make it persistent.
> Unlike 
> pickle
> , 
> json
>  doesn’t support arbitrary objects (just basic types and built-
...
> , the 
> json
>  module always produces a 
> str
> , so files for saves
> and 
> loads should generally use 
> text mode
>  (JSON files are commonly encoded per
...
> names
>  have
> a Unicode story too. Its short version is that 
> str
>  is the norm for file pathnames
> 
> in Python and is recommended for portability. If your code, like all the examples
> so far, uses 
> str
>  for filenames and pathnames, they simply work: Python’s file
...
> pathnames as 
> bytes
>  to skip automatic filesystem encoding in some contexts.
> This 
> bytes
>  mode may come in handy if you need more control over filename
...
> Nevertheless, 
> bytes
>  filenames do work in these tools, as shown in the following
> demo, which runs the same on macOS, Windows, Linux, and Android devices
> tested. As shown, the 
> open
>  function happily accepts text or bytes for a file’s non-
> ASCII pathname (this section’s examples were run in subfolder 
> _filenames
>  in the
...
>  
> # Filesystem default: macOS and Win11
> 'utf-8'
> >>> 
> name1 = 'hÄck
> 1'
...
> name2.encode('utf8')
>  
> # bytes equivalent for name2 str
> b'h\xc3\x84ck\xf0\x9f\x99\x822'
> >>> 
> os.listdir()
>  
> # Make files with str and bytes names
> []
> >>> 
...
> os.listdir()
>  
> # Both show up on the filesystem
> ['hÄck
> 1', 'hÄck
> 2']
> >>> 
...
> 2').read()
>  
> # And can be accessed either way
> 'text2'
> >>> 
> open('hÄck
> 2'.encode('utf8')).read()
...
> For 
> bytes
>  filenames, Python uses UTF-8 encoding on macOS and on Windows
> 
...
> use UTF-8 when converting to and from 
> str
>  (with surrogate escapes for
> encoding errors: see Python’s manuals).
> Whether 
> bytes
>  filenames are a useful tool or neat parlor trick depends on your
> use cases, but keep in mind that 
> bytes
>  filenames will only work in 
> open
>  if their
> encoding matches the expectations of Python or the underlying filesystem. To
> demo, the following passes Latin-1 bytes 
> b'h\xc4ck'
>  
> to 
> open
...
> fails
>  on both
> macOS and Windows as shown (though with a 
> UnicodeDecodeError
>  on the
...
> # open(b'h\xc4ck'.decode('latin1'), 'w')
>  
> # Works: convert to str
> # open(b'h\xc3\x84ck', 'w')
>  
>  
> # Works: use UTF-8 encoding
> By contrast, the preceding code’s 
> open
>  
> works
>  on Linux and Android—which
> later decode the Latin-1 name 
> b'h\xc4ck'
>  to 
> str
>  by UTF-8, as surrogate-
> escaped 
> 'h\udcc4ck'
> . Still, this works only on drives using a Linux-native
> filesystem
> . For both its 
> bytes
>  and escaped 
> str
>  forms, the Latin-1 name fails in
> open
>  on Linux and Android when the target is a removable drive formatted as
> exFAT. Hence, the effect of using arbitrary 
> bytes
>  in 
> open
>  varies by both
...
> The 
> os
>  standard-library module’s directory-listing 
> listdir
>  similarly accepts
> str
>  or 
> bytes
>  and returns a folder’s names in the same form, ready to be used in
...
> The 
> glob
>  module does filename expansion both ways, too (as 
> str
>  or 
> bytes
> ),
> and 
> os.fsdecode
>  decodes filenames per the host’s default automatically:
> >>> 
> glob.glob('h*ck*')
>  
> # str gives str, bytes gives byte
> ['hÄck
> 1', 'hÄck
> 2']
> >>> 
...
> While 
> bytes
>  pathnames work as shown and may have some valid roles, it’s
> important to stress that you’re almost always better off using 
> str
>  strings to name
...
> 除了内建字符串和文件对象，标准库里很多常用的字符串处理工具也遵循 `str` / `bytes` 的分工。`re`、`struct`、`pickle`、`json`、`os`、`glob` 这些模块会根据输入类型返回对应类型，因此在同一个工具里，`str` 和 `bytes` 往往不能随意混用。
>
> `re` 可以处理 `str`、`bytes` 和 `bytearray`，但结果类型会跟随输入类型变化。`struct` 和 `pickle` 面向的是二进制数据，所以通常要配合二进制文件使用；`json` 生成的是文本 `str`，通常应当交给文本文件和显式编码处理。路径名方面，`str` 通常最稳妥，`bytes` 也能用于更底层的文件系统控制，但跨平台兼容性明显更差。
>
> 这正说明 Python 的字符串模型不是单一的“字符串”问题，而是文本、字节、文件、平台编码和标准库工具共同组成的一套系统。理解这一点，后面看文件读写和库函数的行为就会清楚得多。
...
> convoluted story, which we’ve touched 
> on lightly a few times. Now that
> we’ve seen all the players in action, we can finally summarize and finalize
> this thread.
> In short, Python’s default Unicode encoding for both 
> source code
>  and string
> encoding/decoding 
> methods
>  is always UTF-8 everywhere; for 
> filenames
>  can
> be had with 
> sys.getfilesystem
...
> earlier). The 
> filenames
>  default is usually UTF-8, including on Windows as of
> Python 3.6, but is moot if your names are always 
> str
...
>  default for 
> file content
>  is bifurcated most and badly. Contrary to
> Python’s current docs, it’s not simply the result of 
> locale.getencoding()
> on all platforms. Rather, it’s either UTF-8 if the 
> UTF-8 mode
>  introduced in
> Python 3.7 is enabled or else 
> getencoding()
> . This matters on Windows
> today, where 
> getencoding()
>  may be a code-page encoding like cp1252;
> because Unix is usually UTF-8, an 
> open
>  sans 
> encoding
>  isn’t portable for
> non-ASCII content.
> UTF-8 mode
>  addresses this, but it must be enabled today by setting the
> environment variable 
> PYTHONUTF8
>  to 
> 1
>  or using command-line switch 
> -X
> utf8
> . The result of 
> getencoding()
> , added in Python 3.11, is not influenced
> by UTF-8 mode the way that 
> open
>  and the older 
> getpreferred
> encod
...
>  are. Hence, 
> open
> ’s conditional default.
> Both of the 
> locale
>  encoding results may also be influenced by platform,
> environment variables, and command-line switches…
> except
>  on Android,
> which is just plain UTF-8, and on Windows in the future, when 
> Python 3.15
...
>  (e.g., 
> sys.stdout
> ) when they are redirected to
> files on Windows and others (e.g., 
> > 
> output
> .txt
> )…
...
> ; Unix encodings can
> be forced to skip UTF-8 similarly with 
> PYTHONCOERCECLOCALE
> ; and UTF-8
...
> str
>  for filenames and pass explicit encodings to 
> open
>  instead of relying on
> tangled and morphing 
> defaults; and see Python’s manuals for the full, if
> frightening, story.

### 中文翻译

> Python 的默认编码故事比看起来复杂得多。源代码和字符串的编码/解码方法默认总是 UTF-8；文件名的编码可以通过 `sys.getfilesystemencoding()` 查看；而 `open()` 读写文件内容时使用的默认编码，则可能受平台、locale 以及 UTF-8 mode 的影响，Windows 上尤其容易踩坑。
>
> `sys.getdefaultencoding()` 只影响字符串方法层面的默认值，而且在 Python 3 里已经不能再改。文件名的默认编码通常也是 UTF-8，Windows 从 Python 3.6 起也基本如此，但只要你把路径保持为 `str`，通常就不需要过度担心。
>
> 真正麻烦的是文件内容。它并不总是等于 `locale.getpreferredencoding(False)`，而 Python 3.7 之后还可能受到 UTF-8 mode 影响。最稳妥的策略始终不变：读取和写入文本文件时显式传 `encoding=`，不要赌默认值。
...
> 这一节只挑 Unicode 世界里两个最容易绊人的边角：BOM 和规范化。它们都太细碎，不适合在入门阶段展开太深，但只要你处理跨平台文件、编辑器保存选项或来源不明的文本，就很容易碰到。
>
> BOM 负责声明编码和字节序，规范化则负责把“看起来一样、但码点不同”的字符统一到同一种形式。接下来的两个小节，会分别说明 Python 在这些情况下到底做了什么，以及你该如何应对。
...
> byte order marker
>  (BOM) sequence at the start of files to specify data
...
> In 
> UTF-16
> , the BOM is always processed for encoding name 
> utf-16
> ,
> 
> and the more specific encoding name 
> utf-16-le
>  denotes little-endian
> format.
> In 
> UTF-8
> , the more specific encoding name 
> utf-8-sig
>  forces Python
...
> 有些编码会在文件开头写入 BOM（byte order marker），用来表明编码类型，或者说明 UTF-16 / UTF-32 的字节序。Python 的文本模式会在合适的编码名下自动处理 BOM，但前提是你用对名字：`utf-8-sig` 会在读写时处理 UTF-8 BOM，`utf-16` 会自动识别并去掉 BOM，而更具体的 `utf-16-le` / `utf-16-be` 则会把字节序控制权留给你。
>
> 换句话说，BOM 不是“乱码”，而是编码协议的一部分。对于 `utf-16` 这类格式，BOM 甚至是标准的一部分；对于 `utf-8`，它更多只是一个可选标记。你需要做的，不是把 BOM 当成异常去删掉，而是根据真实编码名让 Python 按正确规则解释它。
>
> 所以最稳妥的做法仍然不变：知道文件真实的编码，并在 `open()` 里明确写出来，不要靠猜。
...
> Making BOMs in Text Editors
> Let’s make some files with BOMs to see how 
> this works in practice. We’re going
...
> “code” and “CODE” in a text file named 
> code.txt
>  with encoding type 
> ANSI
> , the
...
> actual bytes stored in the file, including 
> \r
>  in newlines. When it’s read as text,
...
> locale.getpreferredencoding(False)
>  
> # open default: ASCII&Latin-1 superset
> 'cp1252'
> >>> 
...
> 'code\nCODE\n'
> 
> Now, if this file is instead saved as 
> UTF-8 with BOM
>  in Notepad (
> UTF
>  in its
> prior versions), its text is prepended with a three-byte UTF-8 BOM sequence
> (which prints as non-ASCII 
> \x
> NN
>  hex escapes in 
> bytes
>  in Python), and we need
> to give the more specific encoding name 
> utf-8-sig
>  to force Python to skip the
> marker automatically on input (else it prints as a 
> \u
> NNNN
>  Unicode code-point
> escapes in 
...
> 'code\nCODE\n'
> And if the file is stored as 
> UTF-16 BE
>  in Notepad (
> Unicode big endian
> formerly), we get UTF-16 big-endian format data in the file, with two bytes (16
> bits) prepended to record a two-byte BOM sequence. The encoding name 
> utf-
> 16
>  in Python skips the BOM because it is implied (since all UTF-16 files have a
> BOM), and 
> utf-16-be
>  handles the big-endian format but does not skip the
...
> 
> Notepad flux
> : Notepad recently changed its encoding options for saves. Today, it offers ANSI
...
> choices, but this naturally is just the 
> story today. Because this book staunchly refuses to
> become a Notepad doc, translate save options as needed to the Notepad on a PC near you.

### 中文翻译

> 文本编辑器也会制造 BOM，Python 读写时会忠实反映这些差异。像 Notepad 这样的编辑器可以把文件存成 ANSI、UTF-8（有无 BOM）或 UTF-16（大小端）等格式；Python 读文本时会自动做换行转换，读二进制时则会看到原始字节。
>
> 以 `code.txt` 为例，ANSI 保存的 ASCII 内容在 Python 看来很平静；但如果把同样的内容改存成 UTF-8 with BOM，文件头就会多出三个字节，`open(..., encoding='utf-8')` 读出来时也会把 BOM 暴露成字符前缀。换成 UTF-16 BE 后，字节序和 BOM 的影响又会变得更明显。
>
> 这部分的重点不是 Notepad，而是提醒你：同一个“看起来正常”的文本文件，在磁盘上可能对应完全不同的字节序列。跨工具、跨平台交换文件时，编码名必须对齐，否则你看到的只是表象，不是文件本身。
...
> Making BOMs in Python
> The preceding section used Python to read 
> files made in an editor, but the same
...
> encoding name to force the BOM in UTF-8 mode—
> utf-8
>  does not write the
> BOM on output but 
> utf-8-sig
>  does, and the same goes for skipping UTF-8
> BOMs on input. When run on Windows (Unix is the same, but doesn’t add 
> \r
>  to
...
> # utf-8-sig: drops BOM
> 'code\nCODE\n'
> Per this code, although 
> utf-8
>  does not drop the BOM when one is present, data
> without
>  a BOM can be read with both 
> utf-8
>  and 
> utf-8-sig
> —which means you
...
> utf-16
> , the BOM is handled automatically: on 
> output
> ,
> data is written in the platform’s native endianness, and the BOM is always
> written; on 
> input
> , data is decoded per the BOM, and the BOM is always stripped.
...
> sys.byteorder
>  
> # Windows host's default endianness
> 'little'
> >>> 
...
> with BOM-less files, though 
> utf-16
>  requires one on input in order to determine
> byte order:
> >>> 
...
> open('temp.txt', 'rb').read()
>  
> # Okay if BOM not present or expected
...
> more details on the BOM. And if you really want 
> to drop the bomb in Python
...
> Python 自己写文件时也遵循同样的规则。`utf-8` 不写 BOM，`utf-8-sig` 会写；`utf-16` 会按平台字节序写入并自动带上 BOM，读取时也会自动识别；更具体的 `utf-16-le` / `utf-16-be` 则把字节序和 BOM 的处理交给你手动控制。
>
> 这意味着同样是“写文本文件”，编码名不同，结果可能完全不同。`utf-8-sig` 适合你希望兼容 BOM 输入的场景；`utf-16` 适合标准 UTF-16 文本；而当你已经知道自己在和哪种文件打交道时，最好把编码写死，别让默认行为替你做决定。
>
> 如果你要处理带 BOM 的文件，最好先统一约定编码；如果你不确定文件是否带 BOM，`utf-8-sig` 往往是最省心的输入选择。
...
> explaining one way in 
> which it, at least arguably, falls short. In brief, this
> standard 
> failed to standardize
>  code points for a handful of characters: it allows
...
>  (an 
> n
>  augmented with a tilde, commonly used in
> Spanish) can be represented with two different code-point sequences in the
> Unicode standard:
> 
> As a single character with code point 
> \u00F1
> —an augmented 
> n
> As a two-codepoint sequence 
> \u006E
>  and 
> \u0303
> —a naked 
...
> NFC
> , because it
> combines letter and accent. The second is called the 
> decomposed
>  form, known as
> NFD
> , because its parts are split. Despite their glaring difference, the Unicode
> standard mandates that these two forms represent the same character 
> ñ
>  and must
> be treated as such by programs.
> It’s easy to observe these alter egos in Python: the following uses 
> \u…
>  Unicode
> code-point escapes to specify the code points for 
> ñ
>  in both forms (Windows
...
> ̃
> ')
> Nor is this character alone in its split personality. Other characters such as 
> Å
> , 
> è
> ,
...
> '\u00C5', '\u0041\u030A'
>  
> # NFD (1 code point), NFC (2 code points
> )
> ('Å', 'A
> ̊
> ')
...
> '\u00E8', '\u0065\u0300'
>  
> # But it's the same character for both
> ('è', 'e
> ̀
> ')
> >>> 
...
> ̈
> ')
> Importantly, this is not about the 
> encoded
...
> code points
> . Encoded forms (e.g., stored in files) differ,
...
> ̃
> ')
> The unfortunate consequence of this plurality is that it 
> breaks equality testing
> :
> because the same character can be represented in multiple ways, it’s impossible
> to compare with the usual tools. In Python specifically, the 
> ==
>  equal-by-value
...
> L == M
>  
> # But equality says no!
> False
>  
> >>> 
> len(L), len(M)
>  
> # Because their code points differ
> (1, 2)
...
>  
> >>> 
> normalize('NFC', L) == normalize('NFC', M)
>  
> # Common-form equality works
> True
> >>> 
> normalize('NFD', L) == normalize('NFD', M)
>  
> # Either suffices, if the same
> True
...
> users (and really 
> are
>  the same per Unicode) will fail to match by simple equality
> and derail searches and syncs. The same goes for text files obtained from
> arbitrary sources—like most internet content.
> There’s more to this story (e.g., the 
> canonical
>  equivalence of NFC and NFD
> normalized forms is still not the same as 
> compatibility
> , though most programs
...
> web and in Python’s manual set (see the latter’s 
> Unicode HOWTO
>  and its
> coverage of the related string 
> casefold
>  method).
> At the least, though, you now shouldn’t be wholly surprised when your text-
> processing code is bitten by this 
> curious choice of the Unicode standard.

### 中文翻译

> Unicode 还有一个更隐蔽的问题：同一个字符可以有不止一种码点表示。比如 `ñ` 可以是单个码点，也可以是 `n` 加组合附加符号；它们看起来一样，但直接比较会失败，长度也不同。
>
> 规范化的作用，就是把这些“视觉上等价、底层上不同”的字符串归一到同一种标准形式。Python 里的 `unicodedata.normalize()` 就是做这件事的工具，常见的形式有 NFC 和 NFD：前者偏向合成，后者偏向分解。
>
> 在跨平台文件名、网页文本、搜索、同步和比较场景里，这个问题非常常见。换句话说，Unicode 规范化处理的不是编码，而是表示差异；字符已经解码到内存里了，但它们是否属于同一种规范形式，仍然需要程序自己判断。
...
> model fully supports both richer Unicode text via the normal 
> str
>  string type and
> byte-oriented binary data with 
> bytes
>  and 
> bytearray
> .
...
> 本章讲的是 Python 对 Unicode 文本和二进制数据的完整支持。`str` 负责 Unicode 文本，`bytes` 和 `bytearray` 负责原始字节；文件对象会自动编解码文本，标准库里的很多工具也按这条线工作。字符串在这里不只是“字符序列”，而是和文件、编码以及平台默认值绑定在一起的接口。
>
> 我们还看了 BOM 和 normalization 这两个在真实项目里常常会咬人的角落。前者涉及编码和字节序，后者涉及同一字符的多种码点表示；它们都提醒我们，处理文本时不能只看肉眼能不能读出来，还要看底层表示是否一致。
>
> 下一章会转向属性管理，讨论如何在属性访问时自动插入代码。读完本章的测验和答案后，你应该能更稳妥地判断何时用 `str`、何时用 `bytes`，以及何时必须显式写出编码名。
...
> 4
> . 
> What are the main differences between text- and binary-mode files in
> Python?
> 5
> . 
> How would you read a Unicode text file that contains text in a different
...
> 9
> . 
> How large an impact does Python’s text/binary string dichotomy have
> on your code?

### 中文翻译

> 1. Python 中字符串对象类型有哪些，它们各自负责什么？
> 2. Python 的字符串类型在操作上有哪些差异？
> 3. 在 Python 字符串里，怎样编码非 ASCII Unicode 字符？
> 4. 文本模式文件和二进制模式文件的主要差异是什么？
> 5. 如果文件采用的编码与平台默认编码不同，怎样读取 Unicode 文本文件？
> 6. 怎样创建特定编码格式的 Unicode 文本文件？
> 7. 为什么 ASCII 文本可以算作 Unicode 文本的一种？
> 8. Unicode 里的 BOM 和 normalization 各是什么意思？
> 9. Python 的文本/二进制字符串二分，对代码影响有多大？
...
> 1
> . 
> Python has three string types: 
> str
>  (for Unicode text, including ASCII),
> bytes
>  (for binary data with absolute byte values), and 
> bytearray
>  (a
> mutable flavor of 
> bytes
...
> of operations that perform in-place changes. The 
> str
>  and 
> bytes
>  types
...
> 3
> . 
> Non-ASCII Unicode characters can be coded in a 
> str
>  string with both
> hex (
> \xNN
...
> , 
> \UNNNNNNNN
> ) escapes for code points,
> both of which denote character code points. They can also be coded in
> their encoded form as 
> bytes
>  using hex escapes and decoded to text. On
...
> 4
> . 
> Text-mode files assume their content is Unicode text (even if it’s all
...
> as 
> str
>  objects in your script, and the contents of binary files are
> represented as 
> bytes
>  (or 
> bytearray
> ) objects. Text-mode files also
...
> 5
> . 
> To read files encoded in a different encoding than the default for your
> platform, simply pass the name of the file’s encoding to the 
> open
>  built-
> in function; data will be decoded per the specified encoding when it is
> read from the file, and you’ll get back a decoded Unicode-text 
> str
...
> 6
> . 
> To create a Unicode text file in a specific encoding format, pass the
> desired encoding name to 
> open
> ; strings will be encoded per the desired
...
> 7
> . 
> ASCII text is considered to be a kind of Unicode text because its 7-bit
...
> 8
> . 
> The Unicode BOM is a sequence of bytes added to the front of a text
...
> 9
> . 
> The impact of Python’s strings model depends upon the types of strings
> 
...
> str
>  string sans encodings suffices in this case. Moreover, although
> string-related tools in the standard library, such as 
> re
> , 
> struct
> , 
...
> and 
> os
> , may technically use different types in different contexts, the
> effect is largely irrelevant to most programs because Python’s 
> str
>  and
> bytes
>  support almost identical interfaces. On the other hand, if you
> process non-ASCII Unicode text, you’ll need to use 
> str
>  and pass
> encodings to 
> open
> ; if you deal with binary data files, you’ll need to deal
> with content as 
> bytes
>  objects; and if your code must work across many
...
> 1
>  File fine points: most of this chapter’s file examples use filenames relative to, and hence stored in,
> the current directory, so be sure to run them in a directory (a.k.a. folder) where you have 
> permission
> to create files; 
> cd
>  to one in your shell or IDE if needed. This may matter on platforms with major
> storage constraints like Android, but you’ll probably already be in a safe folder in most apps. Also,
> file 
> extensions
>  (e.g., 
> .txt
> ) don’t mean anything to Python and are technically optional; some IDEs
> that hang on to objects for debugging may require manual 
> close
>  calls to flush changes too; and
> purists take note that 
> file
>  is no longer a built-in name in Python and OK to use as a variable here!

### 中文翻译

> 1. Python 有三种字符串类型：`str` 用于 Unicode 文本（包括 ASCII），`bytes` 用于固定字节值的二进制数据，`bytearray` 则是可变版的 `bytes`。通常来说，`str` 代表文本文件中的内容，后两者代表二进制文件中的内容，或者代表已经编码好的文本。
>
> 2. 三种字符串类型在大多数操作上几乎一致：方法调用、序列操作，甚至更大的工具如模式匹配，工作方式都差不多。不同点在于，只有 `str` 支持 `format` 方法和 f-string，`bytearray` 还有原地修改操作；`str` 和 `bytes` 也分别提供编码与解码方法。
>
> 3. 非 ASCII Unicode 字符可以在 `str` 里用十六进制转义 `\xNN` 和 Unicode 转义 `\uNNNN`、`\UNNNNNNNN` 表示；也可以先编码成 `bytes` 再解码回文本。在大多数设备上，你还可以直接把这些字符输入源码，前提是采用 UTF-8 默认或源文件编码声明。
>
> 4. 文本模式文件默认把内容当作 Unicode 文本处理：读取时自动解码，写入时自动编码，并且还会自动转换换行；二进制模式则原样传递字节，不做这些转换。文本模式里的内容通常是 `str`，二进制文件里则是 `bytes` 或 `bytearray`。
>
> 5. 读取不同编码的 Unicode 文本文件时，只要把文件的编码名传给 `open()` 即可，Python 会按指定编码解码并返回没有编码信息的 Unicode `str`。你也可以先二进制读入再手动解码，但对多字节字符来说更容易出错。
>
> 6. 创建特定编码的 Unicode 文本文件时，把想要的编码名传给 `open()`，写入时字符串就会按该编码转换。你也可以先把字符串手动编码成字节，再用二进制模式写入，但通常更麻烦。
>
> 7. ASCII 文本之所以算 Unicode 文本，是因为它 7 位范围内的值是很多 Unicode 编码的子集。也就是说，ASCII 既是 Latin-1 的合法文本，也是 UTF-8 的合法文本，只要这些编码对 ASCII 的表示方式保持一致。
>
> 8. Unicode 的 BOM 是某些编码在文本或文件开头附加的一段字节序列，用来识别编码和字节序。Unicode normalization 则是把字符统一成公共形式，避免同一字符因不同码点序列而比较失败。
>
> 9. 对只处理 ASCII、且平台默认编码也兼容的程序来说，影响通常不大，直接用 `str` 就够了。若要处理非 ASCII 文本，就需要显式指定编码；若要处理二进制数据，就得用 `bytes`；如果代码还要跨平台或处理任意来源内容，显式编码几乎是必需的。今天的文本世界，本来就离不开 Unicode。
>
> 1. 本章的文件示例大多使用相对当前目录的路径，所以运行时要确保自己在有写权限的目录里。扩展名（比如 `.txt`）对 Python 本身没有特殊含义；有些 IDE 需要手动 `close()` 才会刷新输出；另外，`file` 已经不是 Python 的内置名字，这里可以拿来当变量名。
...
- 实际项目中的应用场景
Unicode处理是现代软件开发的基础需求——Web开发中的国际化(i18n)、API开发中的JSON编码、文件处理中的编码转换等都依赖Unicode知识
...
- Python 发展历史背景
Unicode标准从1991年开始制定，Python 2.x中str和unicode是分开的类型，Python 3.x统一为str=Unicode。Python 3.3+引入了PEP 393 flexible string representation
- 高级开发者需要掌握的相关知识
深入学习：codecs模块、locale模块、sys.getfilesystemencoding()、PEP 393、PEP 597(UTF-8 mode)

## 学习建议（Learning Advice）
- 重要程度（4/5）
- 应该掌握到什么程度
掌握Unicode编码原理、str和bytes的区别、文件编码参数的使用
- 后续应该学习哪些相关内容
学习第8章的属性管理、第39章的装饰器，这些章节会进一步深化对Python对象模型的理解
